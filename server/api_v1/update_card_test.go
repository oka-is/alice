...
func TestUpdateCard(t *testing.T) {
	t.Run("when user is a readonly", func(t *testing.T) {
		...
		s.MustPOST(t, "/v1/workspaces/:wid/cards/:cid/update", nil)
	...
	t.Run("when unauthorized", func(t *testing.T) {
		...
		s.MustPOST(t, "/v1/workspaces/:wid/cards/:cid/update", &alice_v1.UpsertCardRequest{})
		require.Equal(t, 403, s.res.Code)
	})
	...
	t.Run("when ok", func(t *testing.T) {
		...
		s.userPolicy.EXPECT().CanWrite().Return(nil)
		s.workspacePolicy.EXPECT().CanManageCard(card).Return(nil)
		...
		s.store.EXPECT().UpdateCardWithItems(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
